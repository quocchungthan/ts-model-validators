...
class ValidatorService {
    ...
    public validate(model: any): void {
        let errors = model[ValidatorService.ERRORS_FIELDS] as ValidationMessage[] || [];
        ...
        for (let rule of validatorRules) {
            ...
            const invalid = rule.isValid(rule.getField(model));
            if (invalid) {
                errors.push({
        ...
        model[ValidatorService.ERRORS_FIELDS] = this.uniquify(errors);
    }
